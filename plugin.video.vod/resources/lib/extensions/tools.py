...
class System(object):
	...
	def restart(self, sleep = True):
		...
		System.execute('RunAddon(%s)' % 'plugin.video.vod')
	...
	@classmethod
	def info(self, value):
		return xbmcaddon.Addon('plugin.video.vod').getAddonInfo(value)
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None, refresh = False):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (System.PluginPrefix, 'plugin.video.vod', parameters)
...
SettingsAddon = xbmcaddon.Addon('plugin.video.vod')
...
class Media(object):
	...
	NameSeasonLong = 'season'#xbmcaddon.Addon('plugin.video.vod').getLocalizedString(32055).encode('utf-8')
	NameSeasonShort = NameSeasonLong[0].upper()
	NameEpisodeLong = 'S'#xbmcaddon.Addon('plugin.video.vod').getLocalizedString(33028).encode('utf-8')
