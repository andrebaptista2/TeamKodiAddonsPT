...
AddonID ='script.milhanomanutencao'
...
def FRESHSTART(mode='verbose'):
    if mode != 'silent': select = xbmcgui.Dialog().yesno("Milhano Manutencao+", 'Are you absolutely certain you want to wipe this install?', '', 'All addons EXCLUDING THIS WIZARD will be completely wiped!', yeslabel='Yes',nolabel='No')
    else: select = 1
    if select == 0: return
    elif select == 1:
...
if action   == None: CATEGORIES()
	...
	xbmc.executebuiltin('Runscript("special://home/addons/script.milhanomanutencao/resources/lib/modules/speedtest.py")')
