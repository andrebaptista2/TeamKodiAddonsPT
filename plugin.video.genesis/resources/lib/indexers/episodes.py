...
class episodes:
    ...
    def calendar(self, url):
        try:
            ...
            if self.trakt_link in url and url == self.progress_link:
                self.blist = cache.get(self.trakt_list2, 720, url, self.trakt_user, self.info_lang)
                self.list = []
                self.list = cache.get(self.trakt_list2, 0, url, self.trakt_user, self.info_lang)
                
            elif self.trakt_link in url and url == self.mycalendar_link:
                self.blist = cache.get(self.trakt_episodes_list, 720, url, self.trakt_user, self.info_lang)
                self.list = []
                self.list = cache.get(self.trakt_episodes_list, 0, url, self.trakt_user, self.info_lang)
                
            elif self.tvmaze_link in url and url == self.added_link:
                urls = [i['url'] for i in self.calendars(dir=False)][:5]
                self.list = []
                for url in urls:
    ...
    def favourites(self):
        try:
            ...
            threads = []
            def f(url): self.list += cache.get(self.tvmaze_list, 720, url, True)
            for i in range(1, 31):
    ...
    def calendars(self, dir=True):
        ...
        for i in range(0, 30):
            try:
                ...
                pass
        if dir == True: self.addDirectory(self.list)
        return self.list
    ...
    def trakt_list(self, url, user):
        ...
        for item in items:
            try:
                ...
                try:
                    if self.info_lang == 'en': raise Exception()

                    item = trakt.getTVShowTranslation(imdb, lang=self.info_lang, season=season, episode=episode,  full=True)

                    title = item.get('title') or title
                    plot = item.get('overview') or plot

                    tvshowtitle = trakt.getTVShowTranslation(imdb, lang=self.info_lang) or tvshowtitle
                except:
                    pass

                itemlist.append({'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': 'Continuing', 'alter': alter, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': '0', 'writer': '0', 'cast': '0', 'plot': plot, 'name': name, 'code': imdb, 'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'tvrage': '0', 'poster': '0', 'banner': '0', 'fanart': '0', 'thumb': '0'})
            except:
                pass
    ...
    def trakt_list2(self, url, user, lang):
        try:
            url += '?extended=full'
        ...
        for item in result:
            try:
                ...
                if tvdb == None or tvdb == '': raise Exception()
                tvdb = re.sub('[^0-9]', '', str(tvdb))
                items.append({'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'tvrage': '0', 'tvshowtitle': tvshowtitle, 'year': year, 'season': season, 'episode': episode})
        ...
        def items_list(i):
            try:
                ...
                self.list.append({'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': status, 'alter': alter, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'writer': writer, 'cast': cast, 'plot': plot, 'name': name, 'code': imdb, 'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'tvrage': '0', 'poster': poster, 'banner': banner, 'fanart': fanart, 'thumb': thumb, 'action': 'episodes'})
                #except:
                #pass
    ...
    def trakt_episodes_list(self, url, user, lang):
        ...
        def items_list(i):
            ...
            try:
                ...
                plot = client.replaceHTMLCodes(plot)
                plot = plot.encode('utf-8')
                self.list.append({'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': status, 'alter': alter, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'writer': writer, 'cast': cast, 'plot': plot, 'name': title, 'code': imdb, 'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'tvrage': '0', 'poster': poster, 'banner': banner, 'fanart': fanart, 'thumb': thumb})
            except:
                pass
    ...
    def tvmaze_list(self, url, limit):
        ...
        for item in items:
            try:
                ...
                plot = client.replaceHTMLCodes(plot)
                plot = plot.encode('utf-8')
                itemlist.append({'title': title, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'status': 'Continuing', 'alter': alter, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': '0', 'mpaa': '0', 'director': '0', 'writer': '0', 'cast': '0', 'plot': plot, 'name': title, 'code': imdb, 'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'tvrage': '0', 'poster': poster, 'banner': '0', 'fanart': thumb1, 'thumb': thumb})
            except:
                pass
