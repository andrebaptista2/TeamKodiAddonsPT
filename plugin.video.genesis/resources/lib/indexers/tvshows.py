...
class tvshows:
    def __init__(self):
        ...
        self.views_link = 'http://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=vote_count.desc&page=1'
        ...
        self.search_link = 'http://api.trakt.tv/search?type=show&query=%s'
        ...
        self.traktratings_link = 'http://api.trakt.tv/users/me/history/shows?limit=300'
    ...
    def trakt_list(self, url, user):
        ...
        for item in items:
            try:
                ...
                if plot == None: plot = '0'
                plot = client.replaceHTMLCodes(plot)
                self.list.append({'title': title, 'originaltitle': title, 'year': year, 'premiered': premiered, 'studio': studio, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'cast': '0', 'plot': plot, 'name': title, 'code': imdb, 'imdb': imdb, 'tmdb': '0', 'tvrage': '0', 'tvdb': tvdb, 'poster': '0', 'banner': '0', 'fanart': '0', 'next': next})
            except:
                pass
