...
V2_API_KEY = '714c3da94ada279969dddb81e8ee9bd74b36b96aa130ccd6d42c410c55a21426'
CLIENT_SECRET = 'ccc3f4dc53e16da141b979770cae3cc5cf03fd3e4af229f2ded582c946b4b976'
...
def __getTrakt(url, post=None):
    try:
        ...
        if getTraktCredentialsInfo():
            headers.update({'Authorization': 'Bearer %s' % control.setting('trakt_token')})
        ...
        opost = {'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET, 'redirect_uri': REDIRECT_URI, 'grant_type': 'refresh_token', 'refresh_token': control.setting('trakt_refresh')}
        ...
        control.setSetting(id='trakt_token', value=token)
        control.setSetting(id='trakt_refresh', value=refresh)
    ...
    try:
        if getTraktCredentialsInfo() == True:
            if control.yesnoDialog(control.lang(32511).encode('utf-8'), control.lang(32512).encode('utf-8'), '', 'Trakt'):
                control.setSetting(id='trakt_user', value='')
                control.setSetting(id='trakt_token', value='')
                control.setSetting(id='trakt_refresh', value='')
            raise Exception()
        ...
        print('info - ' + token)
        control.setSetting(id='trakt_user', value=user)
        control.setSetting(id='trakt_token', value=token)
        control.setSetting(id='trakt_refresh', value=refresh)
        raise Exception()
    except:
...
def getTraktCredentialsInfo():
    user = control.setting('trakt_user').strip()
    token = control.setting('trakt_token')
    refresh = control.setting('trakt_refresh')
...
def manager(name, imdb, tvdb, content):
    try:
        ...
        if select == -1:
            ...
            except: return control.infoDialog(control.lang(30471).encode('utf-8'), heading=str(name), sound=True, icon='ERROR')
        ...
        control.infoDialog(control.lang(30471).encode('utf-8'), heading=str(name), sound=True, icon=icon)
    except:
        return
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.setting('trakt_user').strip())
    return indicators
