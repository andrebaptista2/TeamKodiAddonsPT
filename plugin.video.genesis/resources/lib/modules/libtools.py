...
def check_sources(title, year, imdb, tvdb=None, season=None, episode=None, tvshowtitle=None, premiered=None):
    try:
        ...
        src = sources().checkSources(title, year, imdb, tvdb, season, episode, tvshowtitle, premiered)
...
class libmovies:
    ...
    def add(self, name, title, year, imdb, tmdb, range=False):
        ...
        try:
            ...
            if self.check_setting == 'true':
                src = check_sources(title, year, imdb, None, None, None, None, None)
    ...
    def strmFile(self, i):
        try:
            ...
            file = control.openFile(nfo, 'w')
            file.write(str(nfo_url('movie', i)))
            file.close()
        ...
        for i in items:
            try:
                ...
                if self.check_setting == 'true':
                    if i['episode'] == '1':
                        self.block = True
                        src = check_sources(i['title'], i['year'], i['imdb'], i['tvdb'], i['season'], i['episode'], i['tvshowtitle'], i['premiered'])
