...
class sources:
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.mirror.container.items'

        self.metaProperty = 'plugin.video.mirror.container.meta' 
...
##### >>> needed for the scraper module to work
      
        scraperSetting = control.setting('module.provider') 

        try:
            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.liptonscrapers') and not scraperSetting == 'Lipton':
                ...
                try:
                    self.module_name =     'Exodus' if scrapers[0].split('_')[1].lower() == 'exodus' else \
                                           'mirror' if scrapers[0].split('_')[1].lower() == 'mirror' else \
                                           'mirror' if scrapers[0].split('_')[1].lower() == 'mirror' else 'Lipton'
                except: self.module_name = 'Liptonscrapers'
            else:
                from resources.lib.sources import sources
                self.sourceDict = sources()
                self.module_name = ''
                control.setSetting('module.provider', 'mirror')
        except: return
##################################################################################################################################################
